...
func (*Credential) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case credential.ForeignKeys[0]: // account_credentials
	...
	for i := range columns {
		...
		case credential.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field account_credentials", value)
			} else if value.Valid {
				c.account_credentials = new(int)
				*c.account_credentials = int(value.Int64)
			}
		}
	}
	return nil
