...
func (aq *AccountQuery) QueryKeys() *PrivateKeyQuery {
	query := &PrivateKeyQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withKeys; query != nil {
		...
		query.Where(predicate.PrivateKey(func(s *sql.Selector) {
			s.Where(sql.InValues(account.KeysColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_keys
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_keys" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
